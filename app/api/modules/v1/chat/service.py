...
class ChatService:
    ...
    async def get_conversations(
        ...
        matches_query = self.supabase.table("matches").select("*").or_(
            f"user1_id.eq.{user_id},user2_id.eq.{user_id}"
        ).ilike("status", "active").order("matched_at", desc=True).limit(limit)
        ...
        conversations.sort(
            key=lambda c: c.last_message_at or c.matched_at,
            reverse=True
        )
    ...
    async def get_messages(
        ...
        return MessageListResponse(
            ...
            other_user_read_cursor=self._build_read_cursor_response(
                other_user_cursor
            ) if other_user_cursor else None,
        )
    ...
    async def mark_as_read(
        ...
        if last_read_message_id:
            msg = self.supabase.table("messages").select("id, created_at").eq(
                "id", str(last_read_message_id)
            ).maybe_single().execute()
            message_row = msg.data
        else:
            msg = self.supabase.table("messages").select("id, created_at").eq(
                "match_id", str(match_id)
            ).neq("sender_id", str(user_id)).order("created_at", desc=True).limit(1).execute()
            if msg.data:
        ...
        count_query = self.supabase.table("messages").select("id", count="exact").eq(
            "match_id", str(match_id)
        ).neq("sender_id", str(user_id)).lte("created_at", last_read_at)
    ...
    async def _build_message_response(
        ...
        sender_override: Optional[MessageSender] = None,
    ) -> MessageResponse:
    ...
    async def _build_conversation_preview(
        ...
        profile = self.supabase.table("profiles").select(
            "display_name"
        ).eq("id", matched_user_id).limit(1).execute()
        ...
        # Get last message
        last_msg = self.supabase.table("messages").select("*").eq(
    ...
    def _get_read_cursor(self, match_id: UUID, user_id: UUID) -> Optional[dict]:
        """Fetch read cursor for a user/match."""

        result = self.supabase.table("message_read_cursors").select(
            "match_id, user_id, last_read_message_id, last_read_at, updated_at"
        ).eq("match_id", str(match_id)).eq("user_id", str(user_id)).maybe_single().execute()
    ...
    def _build_read_cursor_response(self, data: dict) -> ReadCursorResponse:
        ...
        if data.get("last_read_at"):
            last_read_at = datetime.fromisoformat(
                data["last_read_at"].replace("Z", "+00:00")
            )
        ...
        return ReadCursorResponse(
            ...
            last_read_message_id=UUID(data["last_read_message_id"]) if data.get("last_read_message_id") else None,
            last_read_at=last_read_at,
